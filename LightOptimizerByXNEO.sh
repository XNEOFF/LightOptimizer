...
# Function to verify the current maximum refresh rate in use
verify_max_refresh_rate() {
  # Switch to superuser and display the current maximum refresh rate
  su -c "settings get system max_refresh_rate"
}

# Function to verify the current minimum refresh rate in use
verify_min_refresh_rate() {
  # Switch to superuser and display the current minimum refresh rate
  su -c "settings get system min_refresh_rate"
}
...
main_menu() {
  ...
  echo "11. Verify the current maximum refresh rate in use"
  echo "12. Verify the current minimum refresh rate in use"
  echo "13. Enhance performance with an optimizer"
  echo "14. Terminate."
}
  ...
  if [[ "$choice" =~ ^[0-9]+$ ]]; then
    case $choice in
      ...
      10)
        ...
        ;;
      11)
        # Call the verify_max_refresh_rate function
        verify_max_refresh_rate
        ;;
      12)
        # Call the verify_min_refresh_rate function
        verify_min_refresh_rate
        ;;
      13)
